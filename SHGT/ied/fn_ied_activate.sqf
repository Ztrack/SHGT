...
SHGT_ied_activateWithNokia = {
	...
	[_ied] spawn {
		...
		if !(_ied isKindOf "MineBase") then {
			...
			if !(isNull attachedTo _ied) then {_car = (attachedTo _ied); detach _ied; _ied setDamage 1; _car setDamage 1;};
			_ied setDamage 1;
		};
	};
};
...
IEDdefusalInteraction = {
	...
	_action = ["DefuseIED","Defuse","\z\ace\addons\explosives\UI\Defuse_ca.paa",
	{
		...
		[_target] spawn {
			...
			sleep 15; 
			if (isNull _ied) then {[_helperObj,SHGT_civInteract_iedDefusalTownRepAdd] call SHGT_fnc_civInteract_changeNearestTownRep; deleteVehicle _helperObj;}; // delete helper & add rep here
		};
...
IEDdefusalInteraction = {
	...
	{[_target] call SHGT_ied_activateWithNokia;},

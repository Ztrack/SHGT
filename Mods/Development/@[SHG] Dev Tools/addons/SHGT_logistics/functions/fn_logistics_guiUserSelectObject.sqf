...
if ((count ((getPosATL player) nearObjects [SHGT_logistics_hqObjectName, SHGT_logistics_radiusToSaveHq]) >0) and ([player, "ACE_Fortify"] call BIS_fnc_hasItem)) then {
	_detected = "HQ";
	_categories = SHGT_logistics_categories getOrDefault ["HQ", "NotFound"];
}; // find if an HQ exists
if ((count ((getPosATL player) nearObjects [SHGT_logistics_fobObjectName, SHGT_logistics_radiusToSaveFob]) >0) and ([player, "ACE_Fortify"] call BIS_fnc_hasItem)) then {
	_detected = "FOB";
	_categories = SHGT_logistics_categories getOrDefault ["FOB", "NotFound"];
}; // find if a FOB exists
if ((count ((getPosATL player) nearObjects [SHGT_logistics_pbObjectName, SHGT_logistics_radiusToSavePb]) >0) and ([player, "ACE_Fortify"] call BIS_fnc_hasItem)) then {
	_detected = "PB";
...
if (count ((getPosATL player) nearObjects [SHGT_logistics_hqObjectName, SHGT_logistics_radiusToNextBase]) >0) then {_categories = _categories - ["CREATION"];};
if (count ((getPosATL player) nearObjects [SHGT_logistics_fobObjectName, SHGT_logistics_radiusToNextBase]) >0) then {_categories = _categories - ["CREATION"];};
if (count ((getPosATL player) nearObjects [SHGT_logistics_pbObjectName, SHGT_logistics_radiusToNextBase]) >0) then {_categories = _categories - ["CREATION"];};
...
if ((_detected isEqualTo "No Bases") and ([player, "ACE_Fortify"] call BIS_fnc_hasItem) and (count ((getPosATL player) nearObjects [SHGT_logistics_fobObjectName, SHGT_logistics_radiusToSavePb*2]) isEqualTo 0)) then {
	_objListCost pushBackUnique [SHGT_logistics_pbObjectName,SHGT_logistics_pbCost,"CREATION"];
};
...
private _uiArray = [];
{
	...
	if (_thisClass isEqualTo SHGT_logistics_hqObjectName) then {_textLeft=["BUILD HQ",[159/225,43/225,104/225,1]]};
	if (_thisClass isEqualTo SHGT_logistics_fobObjectName) then {_textLeft=["BUILD FOB",[159/225,43/225,104/225,1]]};
	if (_thisClass isEqualTo SHGT_logistics_pbObjectName) then {_textLeft=["BUILD PB",[159/225,43/225,104/225,1]]};
	if (_thisClass isEqualTo SHGT_logistics_adminObject) then {_textLeft=["BUILD PERSIST SAVE OBJECT",[159/225,43/225,104/225,1]]};

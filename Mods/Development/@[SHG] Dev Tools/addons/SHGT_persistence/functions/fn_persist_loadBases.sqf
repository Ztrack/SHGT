...
for "_i" from 0 to _numObj-1 do {
	...
	// Create respawn
	_baseType = [];
	if (_objType isEqualTo SHGT_logistics_hqObjectName) then {_baseType = "HQ";};
	if (_objType isEqualTo SHGT_logistics_fobObjectName) then {_baseType = "FOB";};
	if (_objType isEqualTo SHGT_logistics_pbObjectName) then {_baseType = "PB";};
	_baseRespawn = [];
	if (_baseType in SHGT_logistics_addSpawnToBases) then {
		_baseRespawn = [missionNamespace,_pos,_baseName] call BIS_fnc_addRespawnPosition;
	};
	if (_baseType in SHGT_logistics_addPlayerSavingToBases) then {

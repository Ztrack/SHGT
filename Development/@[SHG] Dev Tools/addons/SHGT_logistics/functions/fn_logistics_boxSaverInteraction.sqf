...
[SHGT_logistics_boxSaverInterActionObjectClass, "init", {
	...
	_obj addAction ["Boxfiller - Pull a resupply box", {
		...
		_headline,
		{
			...
			// Addaction to show the box name
			_actionID = _obj addAction [_boxProfileName, {
				params ["_target", "_caller", "_actionId", "_arguments"];
				_caller action ["Gear", _target];
			},nil,6,true,true,"","true",4];
			_txt = format ["<t color='#FFBF00'>%1</t>", _boxProfileName];
			_obj setUserActionText [_actionID, _txt]; // Set interaction use the module defined image

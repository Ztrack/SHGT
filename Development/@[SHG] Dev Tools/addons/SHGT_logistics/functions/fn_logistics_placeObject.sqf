...
// if conditions
//if ((objclassName isEqualTo SHGT_logistics_fobObjectName) and (count ((getPosATL player) nearObjects [SHGT_logistics_fobObjectName, SHGT_logistics_radiusToSaveFob]) >0)) exitWith {systemChat "Too close to existing FOB"};
//if ((objclassName isEqualTo SHGT_logistics_fobObjectName) and (count ((getPosATL player) nearObjects [SHGT_logistics_hqObjectName, SHGT_logistics_radiusToSaveHq]) >0)) exitWith {systemChat "Too close to existing HQ"};
//if ((objclassName isEqualTo SHGT_logistics_hqObjectName) and (count ((getPosATL player) nearObjects [SHGT_logistics_hqObjectName, SHGT_logistics_radiusToSaveHq]) >0)) exitWith {systemChat "Too close to existing HQ"};
//if ((objclassName isEqualTo SHGT_logistics_hqObjectName) and (count ((getPosATL player) nearObjects [SHGT_logistics_fobObjectName, SHGT_logistics_radiusToSaveFob]) >0)) exitWith {systemChat "Too close to existing HQ"};
if ((objclassName isEqualTo SHGT_logistics_hqObjectName) and !(((call BIS_fnc_admin) isEqualTo 2) or (player getVariable [SHGT_logistics_adminTag,false]))) exitWith {systemChat "Only admins can build HQ"};

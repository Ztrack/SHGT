...
if (objclassName isEqualTo SHGT_logistics_hqObjectName) then {baseType = "HQ";};
if (objclassName isEqualTo SHGT_logistics_fobObjectName) then {baseType = "FOB";};
if (objclassName isEqualTo SHGT_logistics_pbObjectName) then {baseType = "PB";};
isNil {
[
    [false,""],
	...
	"Name this base:",
    {
		...
		_baseRespawn = [];
		if (baseType in SHGT_logistics_addSpawnToBases) then {
			_baseRespawn = [missionNamespace,_pos,_baseName] call BIS_fnc_addRespawnPosition;
		};
		if (baseType in SHGT_logistics_addPlayerSavingToBases) then {
